...
void free_frames_init() {
    ...
    if (!frame_list) {
        fprintf(stderr, "Couldnt allocate memory for frame list.\n");
        free(free_frames);
        return;
    }
    
    // Initialize each frame pointer to NULL
    for (int i = 0; i < num_frames; i++) {
        frame_list[i] = NULL;
    }
}
...
PCB* parse_bin_file(char* filename, int page_size) {
    ...
    */
    ...
    uint16_t code_size_raw;
    if (fread(&code_size_raw, sizeof(uint16_t), 1, file) != 1) {
    ...
    pcb->code_size = (code_size_raw >> 8) | (code_size_raw << 8);
    ...
    if (fread(&data_size_raw, sizeof(uint16_t), 1, file) != 1) {
    ...
    // Convert from big-endian (network order) to host order
    pcb->data_size = (data_size_raw >> 8) | (data_size_raw << 8);
    ...
    for (int i = 0; i < pcb->num_pages; i++) {
        ...
        */
        ...
        // Check if frame is already allocated before allocating new one
        if (frame_list[pcb->PTE[i].frame_number] == NULL) {
            frame_list[pcb->PTE[i].frame_number] = malloc(page_size * sizeof(int));
            if (!frame_list[pcb->PTE[i].frame_number]) {
                fprintf(stderr, "Couldnt allocate memory for frame %d\n", pcb->PTE[i].frame_number);
                // Clean up previously allocated frames
                for (int j = 0; j < i; j++) {
                    free(frame_list[pcb->PTE[j].frame_number]);
                    frame_list[pcb->PTE[j].frame_number] = NULL;
                }
                free(pcb->PTE);
                free(pcb->data_segment);
                free(pcb->code_segment);
...
void free_pcb(PCB* pcb) {
    if (pcb) {
        ...
        for (int i = 0; i < pcb->num_pages; i++) {
            if (pcb->PTE[i].valid && pcb->PTE[i].frame_number >= 0) {
                if (frame_list[pcb->PTE[i].frame_number] != NULL) {
                    free(frame_list[pcb->PTE[i].frame_number]);
                    frame_list[pcb->PTE[i].frame_number] = NULL;
                }
                release_frame(pcb->PTE[i].frame_number);
            }
        }
        free(pcb->PTE);
        free(pcb->data_segment);
...
int main(int argc, char* argv[]) {
    ...
    // Free any remaining allocated frames
    for (int i = 0; i < num_frames; i++) {
